...
namespace star
{
    public class StarModule
    {
        ...
        private bool isTimerRunningx = false;
        ...
        public void Attach()
        {
            if (authHD == "none")
            {
                MessageBox.Show("An error occured While Injecting!", "Handler");
            }
            else
            {
                LoadStarlightAPI();
...
";
        ...
        {{
            ...
            {
                MessageBox.Show("Not Attached!", "Handler");
            }
        ...
        private async void InjectTestExecute()
        {
            ...
            if (_webSocket != null && _webSocket.ReadyState == WebSocketSharp.WebSocketState.Open)
            {
                ...
                {
                    MessageBox.Show("Failed to fetch offset-ambigu", "Handler");
                    return;
                }
        ...
        public void TestExecute()
        {
            ...
            {
                MessageBox.Show("Please Inject Starlight!", "Handler");
            }
        }
        ...
        private async void StartAsyncTimer()
        {
            while (isTimerRunning)
            {
                await Task.Delay(300);
                string webSocketUrl = debugURL;
                ListenForAuthorizationHeader(webSocketUrl);
                isTimerRunning = false;
            }
        }

        public bool isInjectedAPI = false;

        private async void injectedVar()
        {
            while (isTimerRunningx)
            {
                if (authHD == "none")
                {
                    isInjectedAPI = false;
        ...
        public async void LaunchDiscord()
        {
